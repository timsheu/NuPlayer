...
@implementation DVRSocketManager
@synthesize hostURL = _hostURL;
@synthesize hostPort = _hostPort;

+ (id)shareInstance {
    static DVRSocketManager* socketManager = nil;
...
- (BOOL)connectHost:(NSString *)hostURL withPort:(NSString *)hostPort withTag:(int)tag{
    ...
    if (isConnected == YES && ![socket.connectedHost isEqualToString:hostURL]) {
        [socket disconnect];
    }
    _hostURL = hostURL;
    _hostPort = hostPort;
...
-(void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err{
    ...
    if (err.code >=4 && err.code < 7) {
        ...
        [self connectHost:localURL withPort:@"80" withTag:tagLocal];
    }
...
//            [_delegate getDeviceInformation:SOCKET_READ_TAG_INFO_STORAGE];
        ...
        for (NSString *s in array){
            ...
            targetString = [json objectForKey:@"value"];
            if (targetString == nil) {
                [socket readDataWithTimeout:-1 tag:SOCKET_READ_TAG_STATUS_RESOLUTION];
                return;
            } else {
                [_delegate updateCameraSettings];
            }
        ...
        NSDictionary *parsedDic = [NSDictionary dictionaryWithDictionary:[DVRRecieveDataParser parseSocketReadData:data withTag:(int)tag]];
        DVRPlayerManager *manager = [DVRPlayerManager sharedInstance];
...
-(BOOL)sendCommand:(NSString *)commandString toCamera:(NSString *)cameraSerial withTag:(int)tag{
    ...
    if (isConnected == NO || ![socket.connectedHost isEqualToString:_hostURL]){
        ...
        return [self connectHost:splitURL withPort:@"8000" withTag:tag];
    }else{
