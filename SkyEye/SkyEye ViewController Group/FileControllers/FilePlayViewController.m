...
@interface FilePlayViewController ()

@end

@implementation FilePlayViewController
...
-(void)playVideoViewWithPath:(NSString *)path seekTime:(float)time{
    ...
    _video = nil;
    BOOL usesTCPFlag = NO;
    NSString *string = [targetDevice objectForKey:@"Transmission"];
    if ([string isEqualToString:@"TCP"]) {
        usesTCPFlag = YES;
    }
    _video = [[RTSPPlayer alloc] initWithVideo:path usesTcp:usesTCPFlag];
    _video.outputWidth = _outletPlayView.bounds.size.width;
    _video.outputHeight = _outletPlayView.bounds.size.height;
